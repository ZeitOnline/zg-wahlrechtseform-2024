...
import useScrollProgressStore, {useWaypoint} from '../ScrollProgressor/store';
...
function Waypoint({identifier}) {
  ...
  const isUnderscroll = useScrollProgressStore((state) => state.isUnderscroll);
  ...
  const Tag = isUnderscroll ? 'div' : 'Card';

  return (
    <div className={cn.wrapper} id={`section-${identifier}`}>
      {error && <div className={cn.error}>{error}</div>}
      <div className={cn.items} data-is-underscroll={isUnderscroll}>
...
function removePlaceholders(innerHtml) {
  return innerHtml.replace(
    /(<div.*?>)?\[KomponentenName\](<\/div>)?/gi,
    '<div class="visuallyHidden">[KomponentenName]</div>',
  );
}

function getReplacement(innerHTML) {
  if (innerHTML.match(/\[KomponentenName\]/gi)) {
    return <div className="replace-with-component" />;
  }
}
