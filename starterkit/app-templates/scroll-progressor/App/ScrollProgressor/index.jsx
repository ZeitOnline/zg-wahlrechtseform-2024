...
function ScrollProgressor({windowHeightOffset = 0.5, ...rest}) {
  const isUnderscroll = toBoolean(rest.isUnderscroll);
  ...
  const setIsUnderscroll = useScrollProgressStore(
    (state) => state.setIsUnderscroll,
  );
  ...
  useIsomorphicLayoutEffect(() => {
    // ein Mal am Anfang direkt setzen, könnte man das ev. noch früher?
    setIsUnderscroll(isUnderscroll);
    if (isUnderscroll && typeof window !== 'undefined') {
      const stickyElementName = document.querySelector('.zg-sticky-container')
        ?.dataset?.stickyElementName;
      const stickyElement =
        stickyElementName &&
        document.querySelector(`div[data-prop-display="${stickyElementName}"]`);
      // Falls der z-index hier nicht passt, einfach anpassen
      if (stickyElement) stickyElement.style.zIndex = 799;
    }
